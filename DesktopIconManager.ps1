...
function Invoke-ElevatedHiddenChange([string[]]$paths, [bool]$hidden) {
    ...
    try {
        $arg = "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$PSCommandPath`" $flag -ListFile `"$listFile`""
...
<Task version="1.2" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "$(Escape-Xml $file)" -ElevatedJob</Arguments>
